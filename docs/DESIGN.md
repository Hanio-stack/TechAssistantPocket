...
4. 通知権限を要求
5. 起床・就寝時刻が未設定なら生活時間を設定（既存ユーザーも一度だけ）
6. Today へ
...
同じ具体的な作業には既存Taskへ予定を追加できる。別の具体的作業は別Taskとして作り、同じカテゴリなら分析・提案の履歴を共有する。
...
- 日付はLifeDayPolicyで起床〜就寝の生活日として判定する。Taskは開始時刻が生活日範囲内のもの、Eventは範囲と重なるものを表示する。現在の生活日を表示中は就寝境界に追従する。


### 7.2 Home v2（実機フィードバック第二弾）

- 正式 UI 参照は `docs/ui/TechSecretaryPocket_Today_Home_v2.jpg`。現在の Task を最も大きいカードにし、カテゴリを主見出し（文字のみ）、タイトルを小見出し、開始〜終了、残り時間、スキップ / 完了を表示する。位置・メモ等のフィールドは追加しない。
...
- 保存後はストアを再取得し Calendar ミラー・通知を更新。旧枠の開始時刻が過去でも、タイトル・カテゴリのみの保存で reschedule や missed 確定を起こさない。所要時間を明示的に操作しなければ、Task本体の推定時間と予定枠の正確な長さをそれぞれ保持する。
...
1. 同一カテゴリに一定数の履歴がある（対象Task自身の履歴がなくてもよい）
2. 現在の曜日 / 時間帯に失敗傾向がある
...
- 正規化タイトル・開始・終了・終日属性・正規化場所が同じ予定は、別カレンダーでも1件にする。識別子は内容キーに含めない。時刻・場所・終日属性が異なる予定と繰り返しの別の回は保持する。既知ミラーを除外してから内容重複を除く。元のEventKitイベントは変更しない。祝日カレンダーが複数あっても、それぞれ取得対象から除外する。
...
- InsightsEngine: 予定成功率、曜日 / 時間帯観測、カテゴリ別集計
...
## 22. 実機フィードバック第三弾 — 日時・カテゴリ・生活日

- TaskEditorは選択中pending枠、ScheduleEditorは指定された枠の保存済み日時・通知を初期表示。開始済みでも現在時刻へ置換しない。未変更保存でrescheduleしない。日時や長さを明示変更した時だけ既存rescheduleを利用する。
- DateTimeFieldsは日付と時・分の選択を分け、時・分を選ぶとDate Bindingを即更新。保存前のフォーカス解除は不要。所要時間を触らなければ正確な枠長を保持する。
- 新規Taskの通知初期値は0分。既存予定のnil/5/15/30分等は保持。既存Taskへの新たな予定追加は設定済み通知デフォルトを使用する。
- CategoryAnalyticsEngineは全Task（Archive含む）の現在カテゴリで既存Occurrenceを再集計する。前後空白除去、空欄は「未分類」。カテゴリ変更は過去分の集約先にも反映するが、保存済み履歴自体は変更しない。
- カテゴリは表示・分析・提案の単位のみ。完了/削除/skip/reschedule/日時/所要時間は個別Task・Occurrenceのまま。提案は対象枠の長さを使用し、現在条件3件以上・成功候補3件以上・改善幅25ポイント以上・空き時間確認・承認を維持する。
- カテゴリ成功率もsuccess / (success + missed)。曜日・時間帯は既存の観測ロジックを再利用し、3件未満なら「データ不足」。実行件数はactualExecutedAtのあるOccurrence数。通常Eventは分析対象外。
- LifeDayPolicyはFoundationの純粋な値型。08:00〜01:30なら起床日の08:00以上、翌01:30未満。07:00〜23:30なら同暦日。就寝後〜起床前は次の生活日を表示する。同時刻の起床/就寝は設定不可。
- DSTはCalendarのローカル日加算を用い、存在しない時刻は前方、重複時刻は最初の時刻へ解決する。時刻の欠落によって起床と就寝が同じ瞬間になる日は空の区間として扱う。
- HomeとCalendar取得は同じ生活日範囲を使用。日付選択・前後ボタン・スワイプは起床日のラベルを切り替える。就寝境界は30秒タイマー/復帰時に追従する。
- Insightsの週・曜日、Suggestionの曜日条件も同じLifeDayPolicyの日ラベルを使う。時間帯は元の時計時刻（00:30なら深夜・早朝）を保持する。睡眠時間帯の履歴も集計から削除せず、次の生活日の所属として扱う。提案する枠は起床〜就寝内へ限定する。
