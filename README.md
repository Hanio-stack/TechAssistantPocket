...
10. 今週の予定成功率とカテゴリ別 Insights
11. カテゴリ詳細で曜日・時間帯の傾向を表示
...
- **Today**: 起床〜就寝の生活日を表示。カテゴリを主見出しにした現在 Task のスキップ / 完了、次の Task / Event、日付切り替え、折りたたみ履歴・未確定、条件を満たしたら振り返り
- **Tasks**: Task 一覧、未スケジュール、今後の Task、追加 / 編集
- **Insights**: 1ページ目は「今週の成功率」と「カテゴリ別」のみ
  - カテゴリをタップすると曜日・時間帯の詳細と個別Taskへの提案を表示
- **Settings**: 生活時間、既定カレンダー、通知など。独立タブにするかは実装時に最小導線を優先して判断

Review は独立タブにしない。

## ドキュメント

- [日時・カテゴリ・生活日の修正と検証](docs/REVISION_3_VALIDATION.md)
